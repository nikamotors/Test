HA$PBExportHeader$n_test_trgt_connectservice.sru
$PBExportComments$Generated Database Connection Service
forward
global type n_test_trgt_connectservice from nonvisualobject
end type
end forward

global type n_test_trgt_connectservice from nonvisualobject
end type
global n_test_trgt_connectservice n_test_trgt_connectservice
...
Choose Case is_connectfrom
	...
	Case IS_USE_INIFILE						/*  Populate Database Connection from INI file  */
		string ls_inifile = "c:\users\fed\desktop\gitrepository2\test_trgt.ini"
		...
		as_server		= ProfileString ( ls_inifile, "Database", "Servername", "otrdb")
		as_dbparm		= ProfileString ( ls_inifile, "Database", "DBParm", "Database='otradnoe',TrustedConnection=1,Provider='SQLNCLI10',BindSPInput=1,TrustServerCertificate=1,ProviderString='DataTypeCompatibility = 0',StaticBind=0")
	...
	Case IS_USE_REGISTRY						/*  Populate Database Connection from Registry  */
		...
		If RegistryGet ( ls_registrykey, "Servername", 	RegString!, as_server ) <> 1 Then
			RegistrySet ( ls_registrykey, "Servername", 	RegString!, "otrdb" )
			RegistryGet ( ls_registrykey, "Servername", 	RegString!, as_server )
		End If
		If RegistryGet ( ls_registrykey, "DBParm", 		RegString!, as_dbparm ) <> 1 Then
			RegistrySet ( ls_registrykey, "DBParm", 		RegString!, "Database='otradnoe',TrustedConnection=1,Provider='SQLNCLI10',BindSPInput=1,TrustServerCertificate=1,ProviderString='DataTypeCompatibility = 0',StaticBind=0" )
	...
	Case IS_USE_SCRIPT							/*  Populate Database Connection from Script  */
		...
		as_logpass		= ""
		as_server		= "otrdb"
		as_dbparm		= "Database='otradnoe',TrustedConnection=1,Provider='SQLNCLI10',BindSPInput=1,TrustServerCertificate=1,ProviderString='DataTypeCompatibility = 0',StaticBind=0"
		as_lock			= ""
		as_autocommit	= "false"
...
on n_test_trgt_connectservice.create
call super::create
TriggerEvent( this, "constructor" )
end on

on n_test_trgt_connectservice.destroy
